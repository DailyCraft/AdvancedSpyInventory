...
public class NMSData extends mc.dailycraft.advancedspyinventory.nms.NMSData {
    ...
    public void putDoubleList(String id, List<Double> value) {
        ...
        value.forEach(v -> tag.add(NBTTagDouble.a(v)));
        data.set(id, tag);
        saveData(data);
    }
    ...
    public void putFloatList(String id, List<Float> value) {
        NBTTagCompound data = getData();
        NBTTagList tag = new NBTTagList();
        value.forEach(v -> tag.add(NBTTagFloat.a(v)));
        data.set(id, tag);
        saveData(data);
    }
    ...
    public ItemStack[] getArray(String id, int size, Function<Integer, Integer> slotConversion) {
        ...
        getData().getList(id, CraftMagicNumbers.NBT.TAG_COMPOUND).stream().map(tag -> (NBTTagCompound) tag)
                .forEach(tag -> array[slotConversion.apply((int) tag.getByte("Slot"))] = CraftItemStack.asBukkitCopy(net.minecraft.server.v1_16_R1.ItemStack.a(tag)));
