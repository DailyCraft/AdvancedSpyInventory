...
public class NMSHandler implements mc.dailycraft.advancedspyinventory.nms.NMSHandler {
    ...
    @Override
    public boolean dropItem(HumanEntity human, boolean all) {
        return ((CraftHumanEntity) human).getHandle().dropItem(all);
    }
        ...
        pipeline.addBefore("packet_handler", handlerId, new ChannelDuplexHandler() {
            ...
            public void channelRead(ChannelHandlerContext context, Object msg) {
                if (msg instanceof PacketPlayInUpdateSign) {
                    ...
                    PacketPlayOutBlockChange packet1 = new PacketPlayOutBlockChange(nmsViewer.world, position);
                    packet1.block = ((CraftBlockData) view.getPlayer().getLocation().getBlock().getBlockData()).getState();
                    nmsViewer.playerConnection.sendPacket(packet1);
