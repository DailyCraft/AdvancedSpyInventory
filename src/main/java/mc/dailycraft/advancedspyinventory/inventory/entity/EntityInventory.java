...
public class EntityInventory<T extends LivingEntity> extends AbstractInventory {
    ...
    public List<ItemStack> getContents() {
        return Lists.newArrayList(((CraftLivingEntity) entity).getHandle().bp());
    }

    @Override
    public ItemStack getItem(int index) {
        if (index >= getSize() - 17 && index <= getSize() - 14)
            return getNonNull(getContents().get(-index + getSize() - 12), InformationItems.values()[-index + getSize() - 12].warning(translation).nms());
            ...
            if (Permissions.ENTITY_HEALTH.has(viewer)) {
                return new ItemStackBuilder(PotionType.INSTANT_HEAL, translation.format("interface.entity.health", entity.getHealth(), entity.getAttribute(Attribute.GENERIC_MAX_HEALTH).getValue()))
                        ...
                        .lore(Permissions.ENTITY_HEALTH_MODIFY_MAX.has(viewer), translation.format("interface.entity.health.modify.max")).nms();
            }
            ...
            return new ItemStackBuilder(Material.BARRIER, Permissions.ENTITY_MODIFY.has(viewer) ? translation.format("interface.entity.clear") : translation.format("interface.entity.close")).nms();
            ...
            switch (entity.getType()) {
                case SHEEP:
                    if (viewer.hasPermission(Permissions.ENTITY_INFORMATION.get(EntityType.SHEEP))) {
                        ...
                        return new ItemStackBuilder(Material.getMaterial(sheep.getColor().name() + "_WOOL"), translation.format("interface.sheep.color", SheepColorInventory.dyeToChatColor(sheep.getColor()) + translation.format("interface.sheep.color." + sheep.getColor().name().toLowerCase()))).modifyLore(viewer, sheep.getType()).nms();
                    }
                ...
                case IRON_GOLEM:
                    if (viewer.hasPermission(Permissions.ENTITY_INFORMATION.get(EntityType.IRON_GOLEM)))
                        return new ItemStackBuilder("MHF_Golem", translation.format("interface.iron_golem." + (((IronGolem) entity).isPlayerCreated() ? "player_creation" : "natural_creation"))).nms();
                ...
                case FOX:
                    if (viewer.hasPermission(Permissions.ENTITY_INFORMATION.get(EntityType.FOX))) {
                        ...
                        return new ItemStackBuilder(fox.getFoxType() == Fox.Type.RED ? Material.SPRUCE_SAPLING : Material.SNOW_BLOCK, translation.format("interface.fox.type", translation.format("interface.fox.type." + fox.getFoxType().name().toLowerCase())))
                                ...
                                .switchLore(viewer, fox.getType()).nms();
                    }
                ...
                case PANDA:
                    if (viewer.hasPermission(Permissions.ENTITY_INFORMATION.get(EntityType.PANDA))) {
                        ...
                        return new ItemStackBuilder(Material.BAMBOO, translation.format("interface.panda.gene", panda.getMainGene()))
                                .lore(translation.format("interface.panda.hidden_gene", panda.getHiddenGene())).nms();
                    }
                ...
                case MAGMA_CUBE:
                    if (viewer.hasPermission(Permissions.ENTITY_INFORMATION.get(EntityType.SLIME))) {
                        return new ItemStackBuilder(Material.SLIME_BLOCK, translation.format("interface.slime.size", ((Slime) entity).getSize()))
                                .modifyLore(viewer, EntityType.SLIME).nms();
                    }
            ...
            if (entity instanceof Tameable && Permissions.ENTITY_TAMED.has(viewer)) {
                if (((Tameable) entity).isTamed()) {
                    if (((Tameable) entity).getOwner() != null)
                        return new ItemStackBuilder(((Tameable) entity).getOwner().getName(), translation.format("interface.entity.tamed", ((Tameable) entity).getOwner().getName())).nms();
                    else
                        return new ItemStackBuilder("MHF_Question", translation.format("interface.entity.tamed", translation.format("interface.entity.unknown"))).nms();
                } else
                    return new ItemStackBuilder(LocalDateTime.now().getMonth() == Month.OCTOBER && LocalDateTime.now().getDayOfMonth() == 31 ? "MHF_Herobrine" : "MHF_Steve", translation.format("interface.entity.untamed")).nms();
            }
    ...
    public void setItem(int index, ItemStack stack) {
        if (index >= getSize() - 17 && index <= getSize() - 14) {
            if (!stack.equals(InformationItems.values()[-index + getSize() - 12].warning(translation).nms())) {
                getContents().set(-index + getSize() - 12, stack);
                ((CraftLivingEntity) entity).getHandle().setSlot(EnumItemSlot.values()[-index + getSize() - 12], stack);
            }
        } else if (index >= getSize() - 12 && index <= getSize() - 11) {
            if (!stack.equals(InformationItems.values()[index - getSize() + 12].warning(translation).nms())) {
                getContents().set(index - getSize() + 12, stack);
    ...
    public void onClick(InventoryClickEvent event, int rawSlot) {
        if (rawSlot >= getSize() - 17 && rawSlot <= getSize() - 14 || rawSlot == getSize() - 12 || rawSlot == getSize() - 11) {
            if (Permissions.ENTITY_MODIFY.has(viewer))
                replaceItem(event, InformationItems.values()[Math.abs(getSize() - rawSlot - 12)].warning(translation).get());
        } else if (rawSlot == getSize() - 8) {
            if (Permissions.ENTITY_HEALTH_MODIFY.has(viewer) && event.getClick() == ClickType.LEFT)
                signInterface((CustomInventoryView) event.getView(), "health", entity.getHealth(), 0d, entity.getMaxHealth(), Double::parseDouble, entity::setHealth);
            else if (Permissions.ENTITY_HEALTH_MODIFY_MAX.has(viewer) && event.getClick() == ClickType.RIGHT)
                signInterface((CustomInventoryView) event.getView(), "health.max", entity.getMaxHealth(), 0d, Double.MAX_VALUE, Double::parseDouble, entity::setMaxHealth);
            ...
            if (entity.getType() == EntityType.SHEEP && viewer.hasPermission(Permissions.ENTITY_INFORMATION_MODIFY.get(EntityType.SHEEP)))
                ...
                signInterface((CustomInventoryView) event.getView(), "slime", ((Slime) entity).getSize(), 1, Integer.MAX_VALUE, Integer::parseInt, result ->
            ...
            if (Permissions.ENTITY_MODIFY.has(viewer)) {
                ...
                shift(event, getSize() - 17, InformationItems.HELMET.warning(translation).get(), current -> current.getItem() instanceof ItemArmor && ((ItemArmor) current.getItem()).b() == EnumItemSlot.HEAD);
                shift(event, getSize() - 16, InformationItems.CHESTPLATE.warning(translation).get(), current -> current.getItem() instanceof ItemArmor && ((ItemArmor) current.getItem()).b() == EnumItemSlot.CHEST);
                shift(event, getSize() - 15, InformationItems.LEGGINGS.warning(translation).get(), current -> current.getItem() instanceof ItemArmor && ((ItemArmor) current.getItem()).b() == EnumItemSlot.LEGS);
                shift(event, getSize() - 14, InformationItems.BOOTS.warning(translation).get(), current -> current.getItem() instanceof ItemArmor && ((ItemArmor) current.getItem()).b() == EnumItemSlot.FEET);
            }
        }
    }
}